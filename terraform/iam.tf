...
resource "aws_iam_role" "verify_movies_in_dynamodb_role" {
  name = "verify_movies_in_dynamodb_role"
...
resource "aws_iam_policy" "verify_movies_in_dynamodb_policy" {
    name        = "verify_movies_in_dynamodb_policy"
    description = "Lambda to verify movies in dynamodb"
    policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
...
resource "aws_iam_role_policy_attachment" "verify_movies_in_dynamodb_attach" {
